...
-(void)initSeatsView:(NSMutableArray *)seatsArray{
   __weak typeof(self) weakSelf = self;
    ZFSeatsView *seatView = [[ZFSeatsView alloc]initWithSeatsArray:seatsArray
        ...
        if (seatBtn.selected) {
            for (int i = 0; i<weakSelf.selecetedSeats.count; i++) {
                ZFSeatButton * seatB = weakSelf.selecetedSeats[i];
                seatB.selected = !seatBtn.selected;
                if (weakSelf.actionBlock)
                    weakSelf.actionBlock(weakSelf.selecetedSeats,allAvailableSeats,errorStr);
            }
