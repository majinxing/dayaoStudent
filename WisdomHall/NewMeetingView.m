...
-(void)addContentClassView:(ClassModel *)classModel{
    ...
    _meetingName.text = _classModel.name;
    _meetingTime.font = [UIFont fontWithName:@"PingFangSC-Regular" size:14];
    [self addSubview:_meetingName];
    ...
    NSMutableString * str = [NSMutableString stringWithFormat:@"%@",_classModel.time];
    ...
    _meetingAttention.text = @"带上纸笔以及笔记本电脑，准备好教研有关资料";
    ...
    _meetingLocation.text = _classModel.typeRoom;
    ...
    if (!_backBtn) {
        ...
        [_backBtn addTarget:self action:@selector(intoClass:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)intoClass:(UIButton *) btn{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(intoClassBtnPressedDelegate:)]) {
        [self.delegate intoClassBtnPressedDelegate:_classModel];
    }
}
