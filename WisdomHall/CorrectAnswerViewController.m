...
-(void)sendImage:(UIImage *)image with:(int)index{
    ...
    __weak CorrectAnswerViewController * weakSelf = self;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==1) {
        ...
        NSString *string = [NSString stringWithFormat:@"%@",q.correctAnswer];
        ...
        if ([string rangeOfString:opt.index].location == NSNotFound) {
            ...
            [cell setThirdImagee:@"Oval6"];
        }
    }else if (indexPath.row == 2+q.qustionOptionsAry.count){
        if ([q.titleType  isEqualToString:@"5"]) {
            //问答。可上传图片
            if (_isAbleAnswer) {
                [cell addFirstTitleTextView:q.correctAnswer withImageAry:nil withIsEdit:YES withIndexRow:(int)indexPath.section];
            }else{
                [cell addFirstTitleTextView:q.correctAnswer withImageAry:nil withIsEdit:NO withIndexRow:(int)indexPath.section];
            }
            [cell changeFirstTitleTextColor];
            //
        }else{
            if (![_testType isEqualToString:@"single"]) {
            ...
            [cell addSeventhTextViewWithStr:q.correctAnswer withIndexRow:_questionNumber];
            
            [cell changeSeventhTextColor];
        }
