...
@property (strong, nonatomic) IBOutlet UIButton *allQuestion;
@end

@implementation SelfAnswerViewController
...
- (IBAction)correctAnswer:(id)sender {
    CorrectAnswerViewController * vc= [[CorrectAnswerViewController alloc] init];
...
-(void)sendImage:(UIImage *)image with:(int)index{
    ...
    __weak SelfAnswerViewController * weakSelf = self;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==1) {
        ...
        [cell addCorrectImage:[self isAnswerCorrect:q]];
...
-(NSString *)isAnswerCorrect:(QuestionModel *)question{
    if ([UIUtils isBlankString: question.correctAnswer] ) {
        return @"";
    }else if ([[NSString stringWithFormat:@"%@",question.questionAnswer] isEqualToString:[NSString stringWithFormat:@"%@",question.correctAnswer]]){
        return @"正确";
    }else{
        return @"错误";
    }
