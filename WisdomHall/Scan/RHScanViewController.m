...
-(void)retunCode:(codeReturn)codeReturnBlock{
    
    self.codeReturnBlock = codeReturnBlock;
}

- (void)viewWillDisappear:(BOOL)animated
{
    self.codeReturnBlock(_dict);
...
- (void)scanResultWithArray:(NSArray<NSString*>*)array
{
    ...
    NSDictionary * dict = [self dictionaryWithJsonString:scanResult];
    _dict = [[NSDictionary alloc] init];
    _dict = dict;
    [self.navigationController popViewControllerAnimated:YES];
//    NSString * s = [dict objectForKey:@"date"];
...
- (void)showError:(NSString*)str withReset:(BOOL)isRest
{
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if ([str isEqualToString:@"请到设置隐私中开启本程序相机权限"]) {
            [self.navigationController popViewControllerAnimated:YES];
            return ;
        }
        [self startScan];
