...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl withBtnTage:(int)btnTag withD:(BOOL)N{
    ...
    if (self) {
        ...
        UIView * vb = [[UIView alloc] initWithFrame:CGRectMake(0, 0, APPLICATION_WIDTH, 50)];
        vb.backgroundColor = [UIColor blackColor];
        vb.alpha = 0.8;
        UIImageView * imaged = [[UIImageView alloc] initWithFrame:CGRectMake(APPLICATION_WIDTH-40, 20, 17, 17)];
        imaged.image = [UIImage imageNamed:@"dtsc"];
        
        UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(APPLICATION_WIDTH-50, 0, 50, 50);
        btn.tag = btnTag;
        [btn addTarget:self action:@selector(deleteImage:) forControlEvents:UIControlEventTouchUpInside];
        if (N) {
            
        }else{
            vb.frame = CGRectMake(0, 0, 0, 0);
            imaged.frame = CGRectMake(0, 0, 0, 0);
...
-(id)initWithFrame:(CGRect)frame withPhotoImage:(UIImage *)image withBtnTag:(int)btnTage withD:(BOOL)N{
    self = [super initWithFrame:frame];
    if (self) {
        UIView * vb = [[UIView alloc] initWithFrame:CGRectMake(0, 0, APPLICATION_WIDTH, 50)];
        vb.backgroundColor = [UIColor blackColor];
...
-(void)deleteImage:(UIButton *) btn{
    NSLog(@"%ld",btn.tag);
    if (self.delegate&&[self.delegate respondsToSelector:@selector(deleteImageButtonPressed:)]) {
        [self.delegate deleteImageButtonPressed:btn];
    }
}
