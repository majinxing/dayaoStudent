...
@property (nonatomic,assign) BOOL NN;
@end
...
-(instancetype)init{
    ...
    if (self) {
        ...
        _number = [[UILabel alloc] initWithFrame:CGRectMake(APPLICATION_WIDTH/2-50, APPLICATION_HEIGHT-70, 100, 15)];
...
//    }
//
    _number.text = [NSString stringWithFormat:@"%lu/%d",(unsigned long)self.index+1,_n];// 数字代替圆点
...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        if ([[self.imagesArray objectAtIndex:index] isKindOfClass:[UIImage class]]) {
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoImage:[self.imagesArray objectAtIndex:index] withBtnTag:(int)index withD:_NN];
        ...
        }else if ([[self.imagesArray objectAtIndex:index] isKindOfClass:[NSString class]]){
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imagesArray objectAtIndex:index] withBtnTage:(int)index withD:_NN];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
            self.photoView=photoV;
...
-(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index deleteImage:(BOOL)N imagesBlock:(NSArray *(^)())imagesBlock{
    ...
    //ImageBrowserViewController *imgBrowserVC =  self;//[[self alloc] init];
   // imgBrowserVC.delegate = self.delegate;
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    _number.text = [NSString stringWithFormat:@"%ld/%d",(long)page+1,_n];
...
-(void)deleteImageButtonPressed:(UIButton *) btn{
    //[self.imagesArray removeObjectAtIndex:btn.tag];
    [self hideScanImageVC];
    if (self.delegate&&[self.delegate respondsToSelector:@selector(imageBrowserVCDeleteImageButtonPressed:)]) {
        [self.delegate imageBrowserVCDeleteImageButtonPressed:btn];
    }
}
#pragma mark - 懒加载
