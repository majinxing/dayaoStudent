...
{
    CGRect frame = CGRectMake(0, 64, APPLICATION_WIDTH, APPLICATION_HEIGHT-64);
    frame.size.height -= kBottomHeight;
    //初始化
...
- (void)loadAssets
{
    ...
    if (_groupInfo.group>0&&_groupInfo.group!=nil) {
...
//    NSLog(@"%@",self.navigationController.viewControllers);
    for (UIViewController *controller in self.navigationController.viewControllers) {
        if ([controller isKindOfClass:[MJXAddCourseOfDiseaseViewController class]]) {
